...
def calc_kinetic(velocity_components):
    '''
        Args:
    ...
    ke = tf.reduce_sum(tf.norm(velocity_components, ord=2, axis=1), axis=0)
...
def calc_temperature(velocity_components):
    '''
    Calculates instantaneous thermodynamic temperature...

        Args:
            velocity_components (tf.array): v_i (x, y, z)
            *** velocity is weighted by sqrt(m) ***
    ...
    mean_ke = tf.reduce_mean(tf.norm(velocity_components, ord=2, axis=1), axis=0)
